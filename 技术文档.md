...
## 项目概述
背景：创建群晖用户，需手动登录群晖创建用户，加入组，，，，等等，因大部分同事小白，链接共享盘，并在桌面生成快捷方式操作困难。
...
### 流程：平台与链接  →  用户发送消息（钉钉应用机器人） → 聊天机器人处理（模版是姓名） → 发送审批卡片 → 管理员审批（完成进入服务层） → 创建账户（Synology NAS SSH远程） → 发送通知（钉钉卡片回调，发送给创建的用户）
